...
impl crate::RenderError for SpanList {
    fn format(&self, filename: &str, src: &str, mut buffer: &mut Vec<u8>, color: bool) {
        ...
        Report::build(kind, (filename, self.span.into_range()))
            .with_config(config)
...
impl crate::RenderError for Error<'_> {
    fn format(&self, filename: &str, src: &str, mut buffer: &mut Vec<u8>, color: bool) {
        ...
        let config = Config::default()
            .with_color(color)
            .with_index_type(IndexType::Byte);
        let span = self.error.span.span.into_range();
        let mut report = Report::build(ReportKind::Error, (filename, span.clone()))
            ...
            .with_labels(self.context(color).into_iter().map(|label| {
                Label::new((filename, label.0.into_range()))
...
impl crate::RenderError for PseudoinstructionError {
    fn format(&self, _: &str, _: &str, mut buffer: &mut Vec<u8>, color: bool) {
        ...
        let config = Config::default()
            .with_color(color)
            .with_index_type(IndexType::Byte);
        let mut report = Report::build(ReportKind::Error, (FILENAME, self.span.into_range()))
            .with_config(config)
            .with_message(self.msg(color))
            .with_label(
                Label::new((FILENAME, self.span.into_range()))
