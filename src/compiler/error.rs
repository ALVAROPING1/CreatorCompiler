...
impl Kind {
    ...
    #[must_use]
    pub fn add_span(self, span: impl Into<SpanList>) -> Error {
        Error {
            span: span.into(),
            kind: Box::new(self),
        }
...
impl crate::RenderError for Error {
    fn format(self, filename: &str, src: &str, mut buffer: &mut Vec<u8>, color: bool) {
        ...
        let (filename, src) = self.span.source.as_ref().map_or((filename, src), |origin| {
            ("<pseudoinstruction expansion>", origin.code.as_str())
        });
        let note_color = color.then_some(Color::BrightBlue);
        let mut report = Report::build(ReportKind::Error, (filename, self.span.span.clone()))
            .with_config(Config::default().with_color(color))
            .with_code(format!("E{:02}", self.kind.error_code()))
            .with_message(self.kind.to_string())
            .with_label(
                Label::new((filename, self.span.span))
        ...
        if let Some(origin) = &self.span.source {
...
pub(crate) trait SpannedErr {
    ...
    ///
    /// * `span`: location in the assembly code that caused the error
    fn add_span(self, span: impl Into<SpanList>) -> Result<Self::T, Error>;
}
